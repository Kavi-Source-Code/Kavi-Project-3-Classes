...
bool Hand::full_house() const{
    ...
    // There's ONLY TWO possible ways (scratch that). Ex: With a list of [8, 8, 7, 8, 7], when sorted, will become [7, 7, 7, 8, 8] OR a list of [8, 7, 7, 8, 8] will become [7, 7, 8, 8,  8]. With this information, we can hopefully make the right conditional.
    
    // Create the conditional in a variable.
    bool combo1 = (copy[0] == copy[1] && copy[1] == copy[2] && copy[3] == copy[4] && copy[0] != copy[3]) || (copy[0] ==copy[1] && copy[2] == copy[3] && copy[3] == copy[4] && copy[0] != copy[3]); //this is stupidly long
...
bool Hand::three_of_a_kind() const{
    ...
    this->sort_by_rank(copy);
    // three of a kind will ONLY be three (ALSO scratch that) combos (e.g. [5, 6, 7, 7, 7], [6, 6, 6, 7, 8]).
    // So, make a variable that holds this.
    bool three_kind = (copy[0] == copy[1] && copy[1] == copy[2]) || (copy[2] == copy[3] && copy[3] ==copy[4]) || (copy[1] == copy[2] && copy[2] == copy[3]); //make this more readable
    return three_kind;
...
int Hand::score(){
    int your_score;
    if (straight_flush()){ //lazy, as there are a lot of if, else if, and else cases.
        your_score = 100;
    }
